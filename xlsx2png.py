...
from utils import auto_direction_html, read_css, write_html, read_structured_data
...
def generate_plotting_code(dataframe: pd.DataFrame, input_user_prompt: str, input_file_path: str, output_plot_image_path: str, previous_code: str = None, previous_error: str = None) -> str:
    prompt = PlotPrompt(
...
if __name__ == "__main__":
    ...
    parser.add_argument("input_user_prompt", type=str, help="User prompt for the AI model")
    ...
    output_plot_image_path = os.path.join(app_settings.OUTPUT_DIR, "plots", output_plot_image)
    ...
    while True:
        ...
        if app_settings.OUTPUT_DEBUG:
            ...
            with open(os.path.join(debug_dir, file_name + time.strftime("%Y%m%d-%H%M%S") + ".py"), "w") as file:
                file.write(code)
        try:
            pythonREPL.run(code)
            break
